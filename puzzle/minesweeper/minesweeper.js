...
!function($){
    ...
    var Game = {
        ...
        complete: function(win) {
            ...
            if(win) {
                $('#status').addClass("alert-success").removeClass("alert-info").removeClass("alert-error").show();
                
$.ajax({
  type: "POST",
  url: 'riddle.php',
  dataType: "json",
  data: {func: 'minesweeper'},
   success:function(data) {
    if(data.status == 'OK'){
      ...
      $('#puzzle-solve').hide();
                ...
                $('#newgame').show();
                $('#status').removeClass("alert-success").removeClass("alert-info").addClass("alert-error").show();
            }
            var el = $("#minesweeper"); 
...
$(function() {
  try {
    $(".prettyprint").text($.trim($("#js").text()));
    prettyPrint();
    $("#content").show();
  }
  catch (e) {
    $("#content").html($('<div class="alert alert-error">').html('<a class="close" data-dismiss="alert">×</a>' + e.message)).show();
  }
});