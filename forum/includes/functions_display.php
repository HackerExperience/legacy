...
//            $clanID = ' 0 OR 1=1';
//        }
//        $sql_where .= '(f.forum_clan = 0 OR f.forum_clan = '.$clanID.')';
        //CUSTOM HE END
...
//CUSTOM HE \/
function get_user_avatar($username, $avatar_type, $avatar_width, $avatar_height, $alt = 'USER_AVATAR', $ignore_config = false)
{
	global $user;
        ...
        $commonPath = '../images/profile/'.md5($username).'.jpg';

        if(!file_exists($commonPath)){
            $commonPath = '';
        }

        $avatar_width = $avatar_height = 90;
        
        if($commonPath == '') return '';
	return '<img src="' . (str_replace(' ', '%20', $commonPath)) . '" width="' . $avatar_width . '" height="' . $avatar_height . '" alt="' . ((!empty($user->lang[$alt])) ? $user->lang[$alt] : $alt) . '" />';
}

function get_user_avatar_custom_he($username){
